...
public class EnergyLevelReport extends Report implements UpdateListener {
	...
	public static int networkFlag = 1;
	public static double residualEnergy = 0;
	public static double networkDeadTime = 0;
	public static double deadNodes = 0;
	...
	public double getNetworkDeadTime() {
		if (networkDeadTime<1) {
			return getSimTime();
		} else {
			return networkDeadTime;		
		}
	}	

	public double getNoOfDeadNodes() {
		return deadNodes;
	}
	...
	private void createSnapshot(List<DTNHost> hosts) {
		...
		int nodesCount = hosts.size();
		double tempDeadNodes = 0;
		double tempResidualEnergy = 0;
		...
		for (DTNHost h : hosts) {
			...
			if (value<1) {
				tempDeadNodes = tempDeadNodes + 1;
			}
			tempResidualEnergy = tempResidualEnergy + value;
			if (flag==1) {
		...
		if ((0.8*nodesCount) <= deadNodes && networkFlag ==1) {
			networkDeadTime = getSimTime();
			networkFlag = 0;		
		}
		residualEnergy = tempResidualEnergy/(nodesCount-deadNodes);
		System.out.println(nodesCount);
		System.out.print("Residual Energy per node: ");
		System.out.println(residualEnergy);
		System.out.print("Network Dead Time ");
		System.out.println(networkDeadTime);
